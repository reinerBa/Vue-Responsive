...
var vueResponsive = (function () {
  ...
  return {
    bind: function (el, binding, vnode) {
      ...
      for (let key in self.__rPermissions[binding.arg]) {
        validInputs.push(key)
        validInputs.push(`hidden-${key}`)
      }

      // to use just one resize-event-listener whoose functions can easy unbound
      self.__responsiveElementInstanceId = ++self.__responsiveElementInstanceId || 1
      var rPermissions = {rId: String(self.__responsiveElementInstanceId)}
      if (typeof self.resizeListeners === 'undefined') {
        self.resizeListeners = {}
        var callInstances = function () {
      ...
      for (let k in self.__rPermissions[binding.arg]) rPermissions[k] = true

      if (preParams[0] === 'hidden-all') {
        preParams.splice(0, 1)
        for (let i in self.__rPermissions[binding.arg]) rPermissions[i] = false
      }
